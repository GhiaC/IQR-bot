...
class Conversation:
    ...
    def show_product_response(json_message):
        result = ''
        for shop in json_message['GetShopResponse']['products']:
            result += \
                '*' + MessageComponent.title + shop['title'] + '* | ' + \
                MessageComponent.text + shop['text'] + ' | ' + \
                MessageComponent.price + shop['price'] + ' | ' + \
                '*' + MessageComponent.discount + '* ' + shop['discount'] + ' | ' + \
                MessageComponent.see_product + '(send:' + str(shop['id']) + ')\n'
        return result
